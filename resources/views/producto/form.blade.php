...
<form action="{{url('/generar/cotizacion')}}" method="post" enctype="multipart/form-data">
{{csrf_field()}}
    <input type="hidden" name="idproducto" value="{{ $producto->PRO_ID }}">
    <div class="form-group">
        <label for="producto">Producto </label><span class="text-red"> * </span>
        <input type="text" class="form-control  {{$errors->has('producto')? 'is-invalid': '' }}" name="producto" value="{{ $producto->PRO_NOMBRE }}">
